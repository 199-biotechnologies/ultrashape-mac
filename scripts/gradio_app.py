...
from ultrashape.utils.device import get_device, get_device_type, get_autocast_dtype, empty_cache

# Global variables to cache the model
MODEL_CACHE = {}


def get_pipeline_cached(config_path, ckpt_path, device=None, low_vram=False):
    if device is None:
        device = get_device()
...
def predict(
    ...
    try:
        ...
        # Use CPU generator for MPS compatibility (MPS doesn't support Generator)
        gen_device = "cpu" if (low_vram or device.type == "mps") else device
