...
# Try to import flash_attn for CUDA acceleration, fall back to PyTorch native
try:
    from flash_attn import flash_attn_varlen_func
    HAS_FLASH_ATTN = True
except ImportError:
    HAS_FLASH_ATTN = False
    print("flash_attn not available, using PyTorch native attention (compatible with Mac/MPS)")
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        if has_padding:
            if HAS_FLASH_ATTN and x.device.type == 'cuda':
                # Use flash attention for CUDA with variable length sequences
                ...
                # Fallback for MPS/CPU: use standard attention with padding mask
                q, k, v = map(lambda t: rearrange(t, 'b n h d -> b h n d', h=self.num_heads), (q, k, v))
                # Create attention mask from y_mask: [b, s2] -> [b, 1, 1, s2]
                attn_mask = y_mask.unsqueeze(1).unsqueeze(2).float()
                attn_mask = attn_mask.masked_fill(~y_mask.unsqueeze(1).unsqueeze(2), float('-inf'))
                attn_mask = attn_mask.masked_fill(y_mask.unsqueeze(1).unsqueeze(2), 0.0)
                context = F.scaled_dot_product_attention(
                    q, k, v, attn_mask=attn_mask
                ).transpose(1, 2).reshape(b, s1, -1)
        else:
            # No padding - use standard scaled dot product attention
            q, k, v = map(lambda t: rearrange(t, 'b n h d -> b h n d', h=self.num_heads), (q, k, v))
            attn_mask = None
            context = F.scaled_dot_product_attention(
                q, k, v, attn_mask=attn_mask
...
class Attention(nn.Module):
    ...
    def forward(self, x, rotary_cos=None, rotary_sin=None):
        ...
        # Use scaled_dot_product_attention (works on CUDA, MPS, and CPU)
