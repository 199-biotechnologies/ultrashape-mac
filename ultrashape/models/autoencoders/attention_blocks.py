...
# Only try to use sageattention if explicitly requested AND on CUDA
if os.environ.get('USE_SAGEATTN', '0') == '1':
    try:
        ...
        if torch.cuda.is_available():
            ...
            print("Using sageattention for CUDA acceleration")
        else:
            print("sageattention requires CUDA, using PyTorch native attention")
    except ImportError:
        print("sageattention not available, using PyTorch native attention")
...
def fps(
    ...
    """
    Farthest Point Sampling - uses torch_cluster on CUDA if available,
    ...
    try:
        ...
        # Fallback: Pure PyTorch FPS implementation for MPS/CPU
        return _fps_pytorch(src, batch, ratio, random_start, batch_size)
...
def _fps_pytorch(
    ...
    This is slower than torch_cluster but works on MPS and CPU.
    """
    device = src.device
    ...
    # Calculate number of points to sample per batch
    n_sample = int(n_points / batch_size * ratio)
    ...
    for b in range(batch_size):
        ...
        n_sel = min(n_sample, n_pts)

        if n_sel == 0:
            continue

        # Initialize distances to infinity
        distances = torch.full((n_pts,), float('inf'), device=device)

        # Pick starting point
        if random_start:
            idx = torch.randint(0, n_pts, (1,), device=device).item()
        else:
            idx = 0
        ...
        for _ in range(n_sel - 1):
            # Update distances
            current_point = points[idx:idx+1]
            dist_to_current = torch.cdist(points, current_point).squeeze(-1)
        ...
        selected_indices.extend(global_indices[batch_selected].tolist())
