...
# Try to import cubvh for CUDA acceleration, fall back to CPU implementation
try:
    import cubvh
    HAS_CUBVH = True
except ImportError:
    HAS_CUBVH = False
    print("cubvh not available, using CPU marching cubes (slower but compatible with Mac/MPS)")
...
class MCSurfaceExtractor(SurfaceExtractor):
    def run(self, grid_logit, *, mc_level, bounds, octree_resolution, **kwargs):
        ...
        if HAS_CUBVH:
            # Use GPU-accelerated sparse marching cubes (CUDA only)
            ...
            # Use CPU-based scikit-image marching cubes (works on Mac/MPS)
            grid_np = grid_logit.cpu().numpy()
            # Replace NaN with a value below the threshold to exclude from mesh
            grid_np = np.nan_to_num(grid_np, nan=mc_level - 1.0)
            try:
                vertices, faces, normals, _ = measure.marching_cubes(
                    grid_np, level=mc_level, method="lewiner"
                )
            except ValueError:
