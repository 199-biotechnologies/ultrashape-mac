    ...
    def __enter__(self):
        ...
        if os.environ.get('HY3DGEN_DEBUG', '0') == '1':
            ...
            if torch.cuda.is_available():
                ...
                self.start.record()
                self._use_cuda_events = True
            else:
                # Use time.perf_counter for MPS/CPU
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        ...
        if os.environ.get('HY3DGEN_DEBUG', '0') == '1':
            ...
            if self._use_cuda_events:
                ...
                self.time = (time.perf_counter() - self._start_time) * 1000  # Convert to ms
            if self.name is not None:
                logger.info(f'{self.name} takes {self.time:.2f} ms')
